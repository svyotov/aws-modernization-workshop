...
+
NOTE: We will not create a *Deployment Stage* to our pipeline because we will leverage an link:version: 0.2
...
phases:
  ...
  post_build:
    commands:
      ...
      - docker push $AWS_ACCOUNT_ID.dkr.ecr.$AWS_DEFAULT_REGION.amazonaws.com/$IMAGE_REPO_NAME:latest[AWS Lambda] function to act as a Kubernetes client to deploy our aplication.
+
