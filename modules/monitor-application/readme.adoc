...
----
+
Step 5:: First we have to update the `log_group_name` so it's uniquely identifiable. The following command will substitute the *[underline]#<UserName>#* paramater in the manifest with your unique username.
+
[source,shell]
----
...
----
+
Step 7:: We can confirm that all the pods chnage to `Running` status by executing the following command:
+
[source,shell]
----
