...
We will leave this process running, and get back to it later in the workshop. So let's open a new `terminal` by pressing the combination keys `[alt+t]` (icon) or pressing the combination keys `[⌥+t]` on the Mac and. Then we'll proceed to the *Containerize Application* module.
